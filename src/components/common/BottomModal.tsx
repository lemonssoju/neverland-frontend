import { useState, useRef, useMemo, useCallback, useEffect, Dispatch, SetStateAction } from 'react';
...
export const CategoryModal = ({ categoryVisible, setCategoryVisible, categories, setCategories }: CategoryModalProps) => {
  ...
  const renderCategoryBackdrop = useCallback(
    (props: any) => <BottomSheetBackdrop style={{flex: 1}} {...props} onPress={() => closeCategory(false)} pressBehavior='close' appearsOnIndex={0} disappearsOnIndex={-1} />,
    [],
  );
...
export const OrderModal = ({ orderVisible, setOrderVisible, order, setOrder }: OrderModalProps) => {
