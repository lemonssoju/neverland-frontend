...
const Contents = ({
  navigation,
}: StackScreenProps<AuthStackParams, 'Contents'>) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center' }}>
      ...
      <R12 style={{ color: GRAY, marginTop: 5 }}>
          ...
          return (
            <View style={{ width: '50%', alignItems: 'center' }}>
              <TouchableOpacity
                ...
                style={{
                  borderRadius: 180,
                  backgroundColor: categories.includes(item)
                    ? MINT
                    : LIGHTBLACK,
                  width: 120,
                  height: 120,
                ...
                {
                  {
                    0: (
                      <MovieIcon
                        color={categories.includes(item) ? LIGHTBLACK : MINT}
                      />
                    ),
                    1: (
                      <DramaIcon
      ...
      <BottomButton
        label="다음"
        onPress={() =>
          navigation.navigate('Preferences', { categories: categories })
        }
      />
    </SafeAreaView>
  );
