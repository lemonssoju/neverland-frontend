...
const ChangePassword = ({ navigation }: StackScreenProps<ProfileStackParams, 'ChangePassword'>) => {
  const [form, setForm] = useState<{ id: string; password: string; newPassword: string; newPasswordConfirm: string; }>({
    id: '',
    password: '',
    newPassword: '',
    newPasswordConfirm: ''
    ...
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={{flex: 1, justifyContent: 'center'}}>
        ...
        <Input
          ...
          onChangeText={(password) => setForm({...form, newPasswordConfirm: password})}
          placeholder='새로운 비밀번호를 한 번 더 입력해주세요.'
          alert='비밀번호가 일치하지 않습니다.'
        />
      </View>
      <BottomButton label='변경 완료' onPress={() => navigation.replace('SettingsHome')} />
    </SafeAreaView>
  )
}
