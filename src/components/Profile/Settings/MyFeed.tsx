...
import { SafeAreaView, FlatList, TouchableOpacity, Image, View } from 'react-native';
...
const FeedItem = ({ category, title, date, rep_pic }: FeedItemProps) => {
  ...
  return (
    <TouchableOpacity style={{flexDirection: 'row', borderRadius: 8, backgroundColor: LIGHTBLACK, marginVertical: 10}} onPress={() => navigation.navigate('FeedDetail')}>
      <Image source={{uri: rep_pic}} style={{width: 90, height: '100%', borderTopLeftRadius: 8, borderBottomLeftRadius: 8}} />
      <View style={{padding: 10, alignItems: 'flex-start'}}>
        <View style={{backgroundColor: MINT, borderRadius: 8, paddingVertical: 5, paddingHorizontal: 8}}>
...
const MyFeed = ({ navigation, route }: StackScreenProps<ProfileStackParams, 'MyFeed'>) => {
    ...
    <SafeAreaView style={{flex: 1}}>
      ...
      <FlatList
        ...
        renderItem={({item}: any) => {
          ...
          return (
            <FeedItem category={category} title={title} rep_pic={rep_pic} date={date} />
          )
        }}
      />
    </SafeAreaView>
