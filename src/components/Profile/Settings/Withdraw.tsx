...
const Withdraw = ({ navigation }: StackScreenProps<ProfileStackParams, 'Withdraw'>) => {
  ...
  const withdrawConfirmAlert = () => {
    Alert.alert(
      ...
      [
        {
          text: "예",
          onPress: () => {navigationToHome.navigate('FeedList')},
          style: 'destructive',
        },
        {
          text: "아니오",
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      ...
      <View style={{flex: 1, justifyContent: 'center'}}>
        <B24 style={{textAlign: 'center', marginBottom: 80}}>정말로 탈퇴하시겠습니까?</B24>
        <Input
          label='비밀번호 입력'
          isRequired
          value={form.password}
        ...
        <Input
          ...
          onChangeText={(password) => setForm({...form, passwordConfirm: password})}
          placeholder='비밀번호를 한 번 더 입력해주세요.'
          alert='비밀번호가 일치하지 않습니다.'
        />
      </View>
      <BottomButton label='변경 완료' onPress={withdrawConfirmAlert} />
    </SafeAreaView>
  )
}
