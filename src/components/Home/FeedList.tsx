...
const FeedList = ({
  navigation,
}: StackScreenProps<HomeStackParams, 'FeedList'>) => {
      ...
      <TopMenuBar>
        {tabs.map((item, index) => {
          return (
            <TopMenuItem
              key={index}
              onPress={() => setEra(item.label)}
              pressed={item.label === era}>
              <B14 style={{ color: item.label === era ? MINT : WHITE }}>
                {item.title}
              </B14>
            </TopMenuItem>
          );
        ...
        ListFooterComponent={() => {
          return (
            <>
              ...
              />
              <B16 style={{ paddingHorizontal: 10, paddingBottom: 10 }}>
                {era}에 이런 일이!
              </B16>
              <FlatList
                data={AppData}
